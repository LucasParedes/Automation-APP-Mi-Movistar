...
import MtvSinComboContratado from '../../../../../pageObjects/MTV/ADM/ActiletDisneyYStar/mtvSinComboContratadoPage'
...
describe("Taggeo Sin Combo Contratado -ingreso.OTP", function () {
    ...
    it("01 - Sin Combo Contratado - WebView", { tags: ['regresion', 'mtvTageo'] }, function () {
        ...
        cy.window().then(win => {
            ...
            cy.get("@event").then((calledEvent) => {
                ...
                let dataL = {
                    event: this.dataLFixture.without_combo.webview.event,
                    pageName: this.dataLFixture.without_combo.webview.pageName,
                    eventFunctionality: this.dataLFixture.without_combo.webview.eventFunctionality,
                    eventOrder: this.dataLFixture.without_combo.webview.eventOrder,
                };
    ...
    // SE LE COMENTA LOS TAGEOS YA QUE MANDA A PAG EXTERNA Y NO SE PUEDE VALIDAR EL DATALAYER
    /* it("02 - Sin Combo Contratado - Botón Actilet mi Cuenta", { tags: ['regresion', 'mtvTageo'] }, function () {
        mtvSinComboContratado.getBotonPrincipal().click()
        // Accede al objeto "window" del navegador y ejecuta una función cuando esté disponible.
        cy.window().then(win => {
            cy.wait(2000)
            // "Espía" la función "push" del objeto "dataLayer" en el objeto "window" y se lo nombra como "@event".
            ...
            cy.get("@event").then((calledEvent) => {
                ...
                // Define un OBJETO con propiedades.
                let dataL = {
                    event: this.dataLFixture.without_combo.button_see_mtv_plans.event,
                    eventCategory: this.dataLFixture.without_combo.button_see_mtv_plans.eventCategory,
                    eventAction: this.dataLFixture.without_combo.button_see_mtv_plans.eventAction,
                    eventLabel: this.dataLFixture.without_combo.button_see_mtv_plans.eventLabel,
                    eventFunctionality: this.dataLFixture.without_combo.button_see_mtv_plans.eventFunctionality,
                    eventOrder: this.dataLFixture.without_combo.button_see_mtv_plans.eventOrder
                };
